...
// component.get( "c.searchAccounts" ) gets a reference to a server-side action—an @AuraEnabled method named searchAccounts. The Apex method must belong to the controller specified on the <aura:component> tag.
// action.setParams(..) takes a JSON object whose keys match the name and type of the Apex method’s parameters.
// action.setCallback(..) provides the JavaScript callback function to handle the asynchronous response from the server-side action. This callback fires the AccountsLoaded event passing the account search results.
